...
/*                                                   +#+                      */
/*   Created: 2022/12/05 06:19:02 by wkonings      #+#    #+#                 */
/*   Updated: 2022/12/06 09:35:47 by wkonings      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int	**move_part_2(int amount, int from, int to, int **crates)
{
	int	backup = amount;
	int move = amount;
...
int	**move(int amount, int from, int to, int **crates)
{
	...
	{
		printf ("top from = [%c], top to = [%c]\n", crates[from][crates[from][0]], crates[to][crates[to][0]]);
		crates[to][crates[to][0] + 1] = crates[from][crates[from][0]]; 
		crates[to][0]++;
		crates[from][crates[from][0]] = 0;
		crates[from][0]--;
		amount--;
...
int	main(void)
{
	...
	while (split[++i])
	{
		...
		// crates = move_part_2(amount, from - 1, to - 1, crates);
	}
